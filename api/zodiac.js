...
// Use your live domain here (or set BASE_URL in Vercel env vars)
...
module.exports = async (req, res) => {
  try {
    ...
    if (sign && SIGN_MESSAGES[sign]) {
      ...
      const buttons = [
        { label: 'Pick Another', action: 'post', target: `${BASE_URL}/api/zodiac?page=1` },
    ...
    const buttons = group.map((s) => ({
      label: capitalise(s),
      action: 'post',
      target: `${BASE_URL}/api/zodiac?sign=${s}`
    }));
    if (pageIndex < SIGN_PAGES.length - 1) {
      buttons.push({ label: 'Next', action: 'post', target: `${BASE_URL}/api/zodiac?page=${pageIndex + 2}` });
    } else {
      buttons.push({ label: 'Start Over', action: 'post', target: `${BASE_URL}/api/zodiac?page=1` });
    }
